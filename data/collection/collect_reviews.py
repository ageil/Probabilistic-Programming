...
def getReviews(offset=0):
    ...
    response = requests.get('https://toolbox.google.com/factcheck/api/search',
    ...
    for idx, r in enumerate(reviews):
        try:
            claimReview = {
                ...
                'reviewRating': {
                    'ratingValue': r[0][3][0][9][0] if (len(r[0][3][0]) > 9 and
                                                        r[0][3][0][9] and
                                                        len(r[0][3][0][9]))
                    else -1,
                    'worstRating': r[0][3][0][9][1] if (len(r[0][3][0]) > 9 and
                                                        r[0][3][0][9] and
                                                        len(r[0][3][0][9]))
                    else -1,
                    'bestRating': r[0][3][0][9][2] if (len(r[0][3][0]) > 9 and
                                                       r[0][3][0][9] and
                                                       len(r[0][3][0][9]))
                    else -1,
                ...
                'tagsRaw': [{'keyword': tag[0], 'probability': tag[1]}
                            for tag in r[0][8]
                            if len(tag) == 3],
                'tagsNamed': [{'keyword': raw_tags[tag[0]],
                               'probability': tag[1]}
                              for tag in r[0][8]
